...
export const downloadInvoice = async (req, res) => {
    try {
        ...
        doc.fillColor(primaryColor)
           .fontSize(25)
           .text('MEDICAL SUPPLY INVOICE', { align: 'center' })
           .moveDown(2);
        ...
        doc.fillColor(textColor)
           ...
           .text(`Invoice Date: ${invoice.invoiceDate.toLocaleDateString()}`, { align: 'right' })
        ...
        doc.fillColor(textColor)
           .fontSize(12)
           .text(`Health Facility: ${invoice.billingDetails.healthFacilityName}`)
           .text(`Address: ${invoice.billingDetails.healthFacilityAddress}`)
           .moveDown();
        ...
        doc.fillColor(textColor)
           .fontSize(12)
           .text(`Supplier: ${invoice.billingDetails.supplierName}`)
           .text(`Address: ${invoice.billingDetails.supplierAddress}`)
           .moveDown(2);
        ...
        tableHeaders.forEach((header, i) => {
            doc.text(header, i * columnWidths[i], tableTop + 20, {
        ...
        // Table Rows
        const tableData = [
            ['Subtotal', `Ksh ${invoice.totalAmount.toFixed(2)}`],
            ['Tax (10%)', `Ksh ${invoice.taxAmount.toFixed(2)}`],
            ['Total Amount', `Ksh ${invoice.netAmount.toFixed(2)}`]
        ];

        doc.fillColor(textColor)
           .fontSize(12);

           const xMargin = 50; // Define X-axis margin

           tableData.forEach((row, i) => {
               doc.text(row[0], xMargin, tableTop + 40 + (i * 20), { 
                   width: columnWidths[0], 
                   align: 'left' 
               }).text(row[1], xMargin + columnWidths[0], tableTop + 40 + (i * 20), { 
